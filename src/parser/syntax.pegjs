...
// ==================
// Equalent to the JS syntax, but with additions:
// – Enums definitions
// – Object definitions
// - Fields (+ get/set accesor definition)
// - Methods (+ get/set accesor definition)
// - try statement
// - match statement
...
GlobalToken     = "var"        !IdentifierPart
                / "global"     !IdentifierPart
...
EnumLiteral
  = def:EnumDefinition __ "{" __ elision:(EnumElision __)? "}" {
      return {
        type: "EnumExpression",
        id: def.id,
        elements: optionalList(extractOptional(elision, 0))
  ...
  / def:EnumDefinition __ "{" __ elements:EnumValueList __ "}" {
      return {
        type: "EnumExpression",
        id: def.id,
        elements: elements
  ...
  / def:EnumDefinition __ "{" __ elements:EnumValueList __ "|" __ elision:(EnumElision __)? "}" {
      return {
        type: "EnumExpression",
        id: def.id,
        elements: elements.concat(optionalList(extractOptional(elision, 0)))
...
EnumValueList
  = head:(
      elision:(EnumElision __)? element:Identifier {
        return optionalList(extractOptional(elision, 0)).concat(element);
      }
    )
    tail:(
      __ "|" __ elision:(EnumElision __)? element:Identifier {
        return optionalList(extractOptional(elision, 0)).concat(element);
      }
    )*
    { return Array.prototype.concat.apply(head, tail); }
...
VariableStatement
  = type:(VarToken / GlobalToken) __ declarations:VariableDeclarationList EOS {
      return {
...
TypecheckDeclaration
  = TypecheckToken __ id:Identifier __ TypingToken __ test:Expression
    {
      return {
        type: "TypecheckDeclaration",
        id: id,
        test: test
      }
    }
